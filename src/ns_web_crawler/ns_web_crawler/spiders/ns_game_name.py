...
class NSGameNameSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for game_dom in games:
            ...
            game = NsGameNameItem()
            cover_dom = game_dom.css(
                "div.ACG-mainbox2 > div.ACG-mainbox2A > div.ACG-mainbox2B > a > img")
            ...
            name_type = response.css(
                "div.ACG-mainbox2 ul li::text").extract_first().strip()
            ...
            game["up_date"] = name_type.split(
                "/")[1] if len(name_type.split('/')) > 1 else ""

            logging.info("game:%s" % game["name_english"])

            yield game

        now_page = response.css(
            "div#BH-pagebtn p.BH-pagebtnA a.pagenow::text").extract_first()
        last_page = response.css(
            "div#BH-pagebtn p.BH-pagebtnA a:last-child::text").extract_first()

        if now_page:
            if int(now_page) < int(last_page):
                next_page = int(now_page) + 1

                next_page_url = self.__replace_url_param(
                    response.url, {'page': next_page})
